...
public:
    bool compare(std::vector<std::string> & cmd_vec)
    {
        ...
        varType typeRes1 = getVarType(val1, true); // use varMode, argument may be a defined variable
        varType typeRes2 = getVarType(val2, true);
        ...
        if(typeRes1 == varType::LIST_VAR) std::cout << "duide" << std::endl;
